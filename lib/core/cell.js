...
const cellRender = (cell, formulaMap, getCellText, cellList = []) => {
  // 扩展1，隐藏单元格值
  if (cell.hidden) {
    return '...';
  }
  ...
  if (cell.type?.toLowerCase() === 'number') {
    // 如果没有值，就直接返回了
    if (cell.text === null || cell.text === undefined || cell.text === '' || !Number.isSafeInteger(cell.text)) {
      return '';
    }

    if (cell.text === 'NaN' || Number.isNaN(cell.text)) {
      return 'NaN';
    }
    ...
    if (cell?.numberFP) {
      // 获取数据处理方式：四舍五入、截取
      const { numberRound = 'round', numberSig } = cell;
      if (numberRound.toLowerCase() === 'round') {
        return Number(cell.text).toFixed(numberSig);
      }
      const temp = 10 ** numberSig;
      return Math.floor(Number(cell.text) * temp) / temp;
    }

    return cell.text;
  }

  // 扩展3，下拉框时，给定 value，自动显示 label
  if (cell.type?.toLowerCase() === 'select' && cell.selectAsync === false && Array.isArray(cell.selectOptions)) {
    for (let i = 0; i < cell.selectOptions.length; i += 1) {
      const item = cell.selectOptions[i];
