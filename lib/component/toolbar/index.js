...
function generateIconItem(it) {
  const { tag, shortcut, value, tip, icon } = it;
  let { el } = it;
  ...
  if (el) {
    btn.el.child(h('div', `${cssPrefix}-icon`).child(el));
  }
...
function generateDropdownItem(it) {
  ...
  dd.change = (item) => this.change(tag, this.getValue(item));
  btn.el.child(dd);

  btn.getValue = function (val) {
    return val;
  };
